...
read_folder <- function(folder) {
  ...
  #this reads in each csv independently into a list
  dat2 <- map(files, read_csv)
  dat2 # this will return dat2 to global environment
...
read_folder <- function(folder, pattern, read_fun) {
  ...
  #this reads in each csv independently into a list
  dat2 <- map(files, read_fun)
  dat2 # this will return dat2 to global environment
  
}

read_folder(folder = fileloc, pattern = "(csv|CSV)$", read_csv)
...
# List columns
# tidyr::nest() embeds tibbles as lists in a column
``` {r nesting}
cars_nested <-
...
# map!
# let's make a linear model for each subset
# then let's pull out the coeffiecients with broom
